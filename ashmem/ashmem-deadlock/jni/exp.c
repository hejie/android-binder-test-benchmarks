...
void* worker(void *arg)
{
	...
	while(it++ < iters) {
		...
		for(i=0;i + PAGE_SIZE < mapsize;i+= 2*PAGE_SIZE){
			//fprintf(stderr,"4\n");
			//fprintf(stderr,"i=%d\n",i);
			if(ashmem_unpin_region(fd,i,PAGE_SIZE) != 0){
				DBG("unpin error");
				}
			}
...
int main(int argc,char**argv)
{
	...
	for(i=0; i<n; i++) {
		...
		threads[i].id = i;
		threads[i].pthreadId = getpid();
	}
	for(i=0; i<n; i++) {
		pid = waitpid((pid_t)-1, NULL, WUNTRACED);
		DBG("child %d exit",pid);
	}
